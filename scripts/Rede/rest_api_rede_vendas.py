...
def extrair_page_key(data: dict):
    """
    Extrai a próxima chave de paginação da API Rede.
...
def consultar_vendas_mes(
    ...
    while True:
        ...
        if transactions:
            todas_transactions.extend(transactions)

        proximo_page_key = extrair_page_key(data)

        print(
            f"VENDAS API | página {pagina} | "
            f"registros página: {len(transactions)} | "
            f"total acumulado: {len(todas_transactions)} | "
    ...
    return todas_transactions, None
